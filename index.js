 ...
 * 2. cleanupOldContactRequests — Deletes contact requests older than configured retention period (runs daily at 3:30 AM UTC)
...
/**
 * Contact Request Retention Period (in days)
 * 
 * Configurable parameter that determines how long contact requests are kept
 * before being automatically deleted by the cleanupOldContactRequests function.
 * 
 * Default: 90 days
 * 
 * To override locally or in CI/CD:
 * - Set environment variable: CONTACT_REQUEST_RETENTION_DAYS=<value>
 * - Or use Firebase CLI: firebase deploy --only functions --set-env-vars CONTACT_REQUEST_RETENTION_DAYS=<value>
 * 
 * Boundary behavior:
 * - Contact requests with createdAt < cutoff timestamp are deleted
 * - Contact requests with createdAt = cutoff timestamp are NOT deleted (preserved)
 * - Contact requests with createdAt > cutoff timestamp are NOT deleted (preserved)
 */
const contactRequestRetentionDays = defineInt("CONTACT_REQUEST_RETENTION_DAYS", {
  default: 90,
  description: "Number of days to retain contact requests before deletion",
});
...
exports.cleanupOldContactRequests = onSchedule(
  ...
  async (_event) => {
    const db      = getDatabase();
    const retentionDays = contactRequestRetentionDays.value();
    ...
    // Query contact requests with createdAt up to and including cutoff
    // Note: endAt(cutoff) includes records with createdAt === cutoff
    // We will filter these out in code to ensure boundary behavior is explicit
    const snapshot = await db
      .ref("contactRequests")
    ...
    snapshot.forEach((child) => {
      totalInspected++;
      const createdAt = child.val()?.createdAt || 0;

      // Explicit boundary behavior: delete only if createdAt < cutoff
      // Records with createdAt === cutoff are NOT deleted (preserved)
      if (createdAt < cutoff) {
    ...
    logger.info("cleanupOldContactRequests: completed", {
      retentionDays: retentionDays,
      totalInspected: totalInspected,
      totalDeleted: totalDeleted,
      totalPreserved: totalInspected - totalDeleted,
      cutoffTimestamp: cutoff,
      cutoffDate: new Date(cutoff).toISOString(),
    });

    if (totalInspected >= BATCH_SIZE) {
      logger.warn(
